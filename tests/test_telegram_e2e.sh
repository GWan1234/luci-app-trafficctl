...
setup_env() {
    ...
    echo "3661.00 7200.00" > "$MOCKDIR/proc/uptime"
    echo "0.50 0.30 0.10 1/100 1234" > "$MOCKDIR/proc/loadavg"
...
network_get_ipaddr() { eval "$1='203.0.113.1'"; }
MOCK
...
case "$*" in
    ...
    *"system.@system"*"hostname"*) echo "TestRouter" ;;
    *) echo "" ;;
esac
MOCK
    chmod +x "$MOCKBIN/uci"
...
case "$*" in
    *%s*) echo "1700000000" ;;
    *%Y-%m-%d*%H:%M*) echo "2026-05-27 12:00" ;;
...
MOCK
    ...
    # Summary script mock (returns fake device list)
    cat > "$MOCKBIN/trafficctl-summary.sh" <<'MOCK'
#!/bin/sh
cat <<'JSON'
[{"ip":"192.168.0.50","name":"Phone","mac":"aa:bb:cc:dd:ee:ff","conns":5,"blocked":false,"wifi_blocked":false,"rate_limit_kbit":0,"shape_kbit":0,"conn_type":"wifi-5G"},{"ip":"192.168.0.100","name":"Laptop","mac":"11:22:33:44:55:66","conns":12,"blocked":true,"wifi_blocked":false,"rate_limit_kbit":0,"shape_kbit":0,"conn_type":"ethernet"}]
JSON
MOCK
...
{"ok":true,"result":[{"update_id":1,"message":{"message_id":1,"chat":{"id":999},"text":"/devices"}}]}
JSON
    ...
    assert_file_contains "devices: keyboard has Phone" "Phone" "$API_LOG"
...
test_cmd_status() {
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":2,"message":{"message_id":2,"chat":{"id":999},"text":"/status"}}]}
JSON

    run_bot_once

    assert_file_contains "status: calls sendMessage" "sendMessage" "$API_LOG"
    assert_file_contains "status: shows Laptop blocked" "Laptop" "$API_LOG"
...
test_cmd_help() {
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":3,"message":{"message_id":3,"chat":{"id":999},"text":"/help"}}]}
JSON
...
test_cb_block() {
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":4,"callback_query":{"id":"cb1","message":{"message_id":10,"chat":{"id":999}},"data":"act:block:192.168.0.50"}}]}
JSON

    run_bot_once

    assert_file_contains "block: calls block script" "trafficctl-block.sh" "$ACTION_LOG"
    assert_file_contains "block: passes IP" "192.168.0.50" "$ACTION_LOG"
...
test_cb_unblock() {
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":5,"callback_query":{"id":"cb2","message":{"message_id":11,"chat":{"id":999}},"data":"act:unblock:192.168.0.100"}}]}
JSON

    run_bot_once

    assert_file_contains "unblock: calls unblock script" "trafficctl-unblock.sh" "$ACTION_LOG"
    assert_file_contains "unblock: passes IP" "192.168.0.100" "$ACTION_LOG"
...
test_cb_limit() {
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":6,"callback_query":{"id":"cb3","message":{"message_id":12,"chat":{"id":999}},"data":"act:limit:192.168.0.50:5000"}}]}
JSON

    run_bot_once

    assert_file_contains "limit: calls ratelimit script" "trafficctl-ratelimit.sh" "$ACTION_LOG"
    assert_file_contains "limit: passes IP" "192.168.0.50" "$ACTION_LOG"
...
test_cb_shape() {
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":7,"callback_query":{"id":"cb4","message":{"message_id":13,"chat":{"id":999}},"data":"act:shape:192.168.0.50:10000"}}]}
JSON

    run_bot_once

    assert_file_contains "shape: calls shape script" "trafficctl-shape.sh" "$ACTION_LOG"
    assert_file_contains "shape: passes IP" "192.168.0.50" "$ACTION_LOG"
...
MOCK
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":8,"callback_query":{"id":"cb5","message":{"message_id":14,"chat":{"id":999}},"data":"act:block:192.168.0.50"}}]}
JSON
...
test_unauthorized_chat() {
    ...
    # Message from wrong chat_id (888 instead of 999)
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":9,"message":{"message_id":5,"chat":{"id":888},"text":"/devices"}}]}
JSON
...
test_cb_invalid_ip() {
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":10,"callback_query":{"id":"cb6","message":{"message_id":15,"chat":{"id":999}},"data":"act:block:192.168.0;rm -rf /"}}]}
JSON
...
test_cb_wifi_block() {
    ...
    cat > "$MOCKDIR/getUpdates_response.json" <<'JSON'
{"ok":true,"result":[{"update_id":11,"callback_query":{"id":"cb7","message":{"message_id":16,"chat":{"id":999}},"data":"act:wblock:192.168.0.50"}}]}
JSON

    run_bot_once

    assert_file_contains "wblock: calls macfilter-add" "trafficctl-macfilter-add.sh" "$ACTION_LOG"
    assert_file_contains "wblock: passes IP" "192.168.0.50" "$ACTION_LOG"
